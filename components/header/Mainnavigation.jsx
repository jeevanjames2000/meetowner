    ...
    return (
        <>
            <div className="flex flex-col md:flex-row md:items-center justify-between px-5 md:px-0 gap-5 md:gap-10">
                <Link href="/dashboard" className={`font-semibold w-fit text-sm 2xl:text-[18px] 3xl:text-[20px] 4xl:text-[22px] text-[#1D3A76] ${isActive('/dashboard') ? 'border-b-2 border-[#1D3A76]' : ''}`}> Dashboard</Link>
                <Link href="/enquiry" className={`font-semibold w-fit text-sm 2xl:text-[18px] 3xl:text-[20px] 4xl:text-[22px] text-[#1D3A76] ${isActive('/enquiry') ? 'border-b-2 border-[#1D3A76]' : ''}`}> Enquires</Link>
                <Link href="/listings" className={`font-semibold w-fit text-sm 2xl:text-[18px] 3xl:text-[20px] 4xl:text-[22px] text-[#1D3A76] ${isActive('/listings') ? 'border-b-2 border-[#1D3A76]' : ''}`}>Listings</Link>
                <Link href="/packages" className={`font-semibold w-fit text-sm 2xl:text-[18px] 3xl:text-[20px] 4xl:text-[22px] text-[#1D3A76] ${isActive('/packages') ? 'border-b-2 border-[#1D3A76]' : ''}`}>Packages</Link>
                <div
                    onMouseEnter={() => setOpen(true)}
                    onMouseLeave={() => setOpen(false)}
                    className="relative"
                >
                    <Link href="#" className=" font-semibold text-sm 2xl:text-[18px] 3xl:text-[20px] 4xl:text-[22px] text-[#1D3A76] w-fit">
                        More
                    </Link>
                    <AnimatePresence>
                        {open && (
